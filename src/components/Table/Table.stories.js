...
const rowStyles = css`
	border-bottom: 1px solid ${(props) => props.theme.global.colors['light-0']};
`
...
storiesOf('Table', module)
	...
	.add('css', () => (
		<Theme>
			<Table caption="Planets of the solar system">
				<Table.Header>
					<Table.Row css={rowStyles}>
						<Table.HeaderCell css={cellStyles}>Name</Table.HeaderCell>
						<Table.HeaderCell css={cellStyles}>Volume (km3)</Table.HeaderCell>
						<Table.HeaderCell css={cellStyles}>Density (g/cm3)</Table.HeaderCell>
						<Table.HeaderCell css={cellStyles}>Mass (kg)</Table.HeaderCell>
					</Table.Row>
				</Table.Header>
				<Table.Body>
					{planets.map((planet) => (
						<Table.Row key={planet.name} css={rowStyles}>
							<Table.Cell css={cellStyles}>{planet.name}</Table.Cell>
							<Table.Cell css={cellStyles}>{planet.volume}</Table.Cell>
							<Table.Cell css={cellStyles}>{planet.density}</Table.Cell>
							<Table.Cell css={cellStyles}>{planet.mass}</Table.Cell>
						</Table.Row>
					))}
				</Table.Body>
				<Table.Footer>
					<Table.Row css={rowStyles}>
						<Table.Cell css={cellStyles}>Foot 1</Table.Cell>
						<Table.Cell css={cellStyles}>Foot 2</Table.Cell>
						<Table.Cell css={cellStyles}>Foot 3</Table.Cell>
						<Table.Cell css={cellStyles}>Foot 4</Table.Cell>
					</Table.Row>
				</Table.Footer>
			</Table>
		</Theme>
